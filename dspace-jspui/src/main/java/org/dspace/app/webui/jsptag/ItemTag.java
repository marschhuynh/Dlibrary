            		...
            		for (int i = 0; i < bundles.length; i++)
            		{
            			...
            			for (int k = 0; k < bitstreams.length; k++)
            			{
            				...
            				if (!bitstreams[k].getFormat().isInternal())
            				{
            					...
            					String bsUrl = "http://113.174.172.107:8080/pdf.js/web/viewer.html?file=http://http://113.174.172.107:8080/bitstream/" 
            							+ item.getHandle() + "/" 
            							+ bitstreams[k].getSequenceID() + "/"
            							+ "/" + UIUtil.encodeBitstreamName(bitstreams[k].getName(), Constants.DEFAULT_ENCODING);
            							out.print("<a class=\"btn btn-primary\" target=\"_blank\" href=\'"+bsUrl+"'>Xem truc tuyen</a>");
